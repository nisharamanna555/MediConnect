        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                if(currentPage < 6) {
                    let i = 1;
                    while(i <= maxPage && i <= 5) {
                        if(i === currentPage) { pagination.innerHTML += "<a class='pageNum' href='/physician?page="+ i +"'><b>" + i + "</b> </a>"; }
                        else { pagination.innerHTML += "<a class='pageNum' href='/physician?page="+ i +"'>" + i + " </a>"; }
                        i++;
                    }
                    if(maxPage > 5) {
                        pagination.innerHTML += "<a class='pageNum' href='/physician?page=6'>></a>";
                    }
                } else {
                    pagination.innerHTML += "<a class='pageNum' href='/physician?page="+ (currentPage-3) +"'>< </a>";
                    let i = currentPage - 2;
                ...
                for(let id in data) {
                    if(i < currentPage*numPerPage && i >= (currentPage-1)*numPerPage) {
                        ...
                        row.addEventListener('click', function() {
                            window.location.href = '/physicianPatientDetails?patient_id='+id.toString();
                        });
